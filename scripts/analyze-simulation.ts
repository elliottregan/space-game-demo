...
function runSingleGame(seed: number): RunResult {
  ...
  while (!api.game.isGameOver() && solsRun < 10000) {
    ...
    if (currentSol % SNAPSHOT_INTERVAL === 0) {
      ...
      flowTimeline.push({
        sol: currentSol,
        netFood:
          (resources.production.food ?? 0) - (resources.consumption.food ?? 0),
        netOxygen:
          (resources.production.oxygen ?? 0) -
          (resources.consumption.oxygen ?? 0),
        netWater:
          (resources.production.water ?? 0) -
          (resources.consumption.water ?? 0),
        netPower:
          (resources.production.power ?? 0) -
          (resources.consumption.power ?? 0),
        netMaterials:
          (resources.production.materials ?? 0) -
          (resources.consumption.materials ?? 0),
      })
    }
  ...
  if (outcome === "victory") {
    ...
    const reason = victoryState.reason?.toLowerCase() ?? ""
    if (reason.includes("food") || reason.includes("starv"))
      defeatReason = "starvation"
    else if (reason.includes("oxygen") || reason.includes("suffocat"))
  ...
  return {
    ...
    buildingFirstBuiltSol:
      Object.keys(buildingFirstBuiltSolRecord).length > 0
        ...
        : undefined,
    techCompletedSol:
      Object.keys(techCompletedSolRecord).length > 0
        ? techCompletedSolRecord
        : undefined,
    defeatSol,
    resourcesAtDeath,
    blockedDecisions:
      blockedDecisions.length > 0 ? blockedDecisions : undefined,
 ...
 */
function computeVictoryTimeStats(
  victoryTimes: number[],
): VictoryTimeStats | null {
  ...
  for (const bucket of buckets) {
    const count = victoryTimes.filter(
      (t) => t > prevBucket && t <= bucket,
    ).length
...
function computeVictoryDefeatComparison(
  ...
  const avgPeakPopVictory =
    victories.reduce((a, r) => a + r.peakPopulation, 0) / victories.length
  const avgPeakPopDefeat =
    defeats.reduce((a, r) => a + r.peakPopulation, 0) / defeats.length

  const avgTechCountVictory =
    victories.reduce((a, r) => a + r.techsResearched.length, 0) /
    victories.length
  ...
  const avgBuildingCountVictory =
    victories.reduce(
      (a, r) => a + Object.values(r.buildingsBuilt).reduce((s, c) => s + c, 0),
      0,
    ) / victories.length
  ...
  const firstBuildingTiming: Record<string, {
    victory: number | null
    defeat: number | null
  }> = {}
...
function computeCorrelations(results: RunResult[]): CorrelationAnalysis {
  ...
  const firstFarmSols = results.map(
    (r) => r.buildingFirstBuiltSol?.["basic_farm"] ?? 100,
  )
...
function computeBottlenecks(results: RunResult[]): BottleneckAnalysis {
  ...
  for (const result of results) {
    ...
    for (const blocked of result.blockedDecisions) {
      ...
      blockReasons[key][blocked.reason] =
        (blockReasons[key][blocked.reason] ?? 0) + 1
    }
  }
...
function computeEventImpact(results: RunResult[]): EventImpactAnalysis {
  ...
  const baselineVictoryRate =
    (results.filter((r) => r.outcome === "victory").length / results.length) *
    100
  ...
  const events = Object.entries(eventStats)
    ...
    .map(([eventId, stats]) => {
      const victoryRate =
        stats.total > 0 ? (stats.victories / stats.total) * 100 : 0
      return {
        eventId,
        count: stats.total,
        victoryRate,
...
function computeCrisisTimeline(results: RunResult[]): CrisisTimelineAnalysis {
  ...
  let firstCrisisTiming: { median: number min: number max: number } | null =
    null
...
function computeOutliers(victories: RunResult[]): OutlierAnalysis {
  ...
  return {
    ...
    percentage:
      victories.length > 0 ? (slowRuns.length / victories.length) * 100 : 0,
    avgTime:
      slowRuns.length > 0
...
async function writeJsonOutput(
  ...
  const resourceTimeline: ResourceSnapshot[] = []
  for (const [sol, snapshots] of Array.from(timelineBySOL.entries()).sort(
    (a, b) => a[0] - b[0],
  )) {
  ...
  const output: AnalysisOutput = {
    ...
    stats: {
      ...
      victoryDefeatComparison: computeVictoryDefeatComparison(
        victories,
        defeats,
      ),
...
async function main(): Promise<void> {
  ...
  output(
    `  Mean: ${(peakPops.reduce((a, b) => a + b, 0) / peakPops.length).toFixed(1)}`,
  )
  ...
  const slowRuns = results.filter(
    (r) => r.outcome === "victory" && r.finalSol > 550,
  )
  ...
  if (victoryTimes.length > 0) {
    ...
    output(
      `  Gap: ${Math.min(...victoryTimes) - 480} sols (setup/building time)`,
    )
  }
  ...
  output("-".repeat(40))
  output(
    `  Win Rate: ${((victories.length / results.length) * 100).toFixed(0)}%`,
  )
...
function analyzeVictoriesVsDefeats(
    ...
    if (victoryTimes.length > 0 || defeatTimes.length > 0) {
      ...
      const avgDefeat =
        defeatTimes.length > 0
          ...
          : "N/A"
      output(
        `    ${building.padEnd(20)} ${avgVictory.padStart(7)}    ${avgDefeat.padStart(6)}`,
      )
    }
  }
  ...
  if (defeats.length > 0) {
    const defeatSols = defeats.map((r) => r.defeatSol ?? r.finalSol)
    const sortedDefeatSols = [...defeatSols].sort((a, b) => a - b)
    const medianDefeatSol =
      sortedDefeatSols[Math.floor(sortedDefeatSols.length / 2)] ?? 0
...
function analyzeCorrelations(results: RunResult[]): void {
  ...
  output(
    `    Tech Count:            ${techCorr.toFixed(3)} ${
      techCorr > 0.1 ? "(more = better)" : ""
    }`,
  )
  output(
    `    Peak Population:       ${popCorr.toFixed(3)} ${
      popCorr > 0.1 ? "(higher = better)" : ""
    }`,
  )
  output(
...
function analyzeEventImpact(results: RunResult[]): void {
  ...
  // Sort by frequency
  const sortedEvents = Object.entries(eventStats).sort(
...
function analyzeCrisisTimeline(results: RunResult[]): void {
  ...
  for (const [type, data] of Object.entries(crisisByType)) {
    ...
    output(`\n  ${type}:`)
    output(
      `    Total: ${total} events (${totalWarnings} warning, ${totalCritical} critical)`,
    )
