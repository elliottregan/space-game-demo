...
- The AI uses a multi-step greedy policy each turn: (0) slots beneficial policy candidates drawn that turn (when a slot is free or the card stacks a match; prefers draw/influence/storage cards and never spends a fresh slot on Conscription's +1 Dissent cost), (1) builds the column with the highest **marginal (leveled)** value, (2) multi-card commit (straight, full-house, four-of-a-kind, three-of-a-kind, two-pair, pair — lands first, then roles), (3) single-card placement, (4) store one land toward a straight, (5) end turn. It never replaces stored cards (no-churn rule), and it never removes a slotted policy. Policy power flows through `effectiveRules`, so a slotted Mobilize/Mandate/Stockpile raises the AI's effective hand/influence/storage on subsequent turns. Pattern zeros in the output indicate **deck-composition limits**, not AI limits — e.g. Generation Ship's 2-ideology deck rules out trips/quads/full-house entirely; straight-flush and royal-flush require card distributions the starting decks rarely provide.
