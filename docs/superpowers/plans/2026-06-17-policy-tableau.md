...
- [x] Extend the view-model: each built node gains `majorities: (Ideology | null)[]` — **one entry per build** (`projectMajority` of each unlock's cards), *not* a single pooled majority, so the rendered big-counter tally stays in lockstep with `ideologyInfluence` (which counts per-unlock) even when the same pattern is built more than once. Each node also gains `cardIdeologies: Ideology[]` (per-card, wild excluded) for the small counters, and the tree gains an `influence: Record<Ideology, number>` summary (from `ideologyInfluence`). Tests assert a built pair of two Solidarity lands yields a `"solidarity"` majority entry and that the big-counter tally equals the influence readout under repeat builds.
